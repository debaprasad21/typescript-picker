...
@Directive({
    selector: 'input[dtDateTime]',
    exportAs: 'dtDateTimeInput',
    host: {
...
export class DateTimeInputDirective<T>
    implements
        OnInit,
        AfterContentInit,
        OnDestroy,
     ...
     * */
    @Input()
    set dtDateTime(value: DateTimeComponent<T>) {
     ...
     */
    @Input()
    set dtDateTimeFilter(filter: (date: T | null) => boolean) {
    ...
    constructor( private elmRef: ElementRef,
        ...
        @Optional() @Inject(DATE_TIME_FORMATS) private dateTimeFormats: DateTimeFormats ) {
        if (!this.dateTimeAdapter) {
            throw Error(
                `DtDateTimePicker: No provider found for DateTimePicker. You must import one of the following ` +
                    `modules at your application root: DTNativeDateTimeModule, DtMomentDateTimeModule, or provide a ` +
        ...
        if (!this.dateTimeFormats) {
            throw Error(
                `DtDateTimePicker: No provider found for DATE_TIME_FORMATS. You must import one of the following ` +
                    `modules at your application root: DTNativeDateTimeModule, DtMomentDateTimeModule, or provide a ` +
                    `custom implementation.`
            );
        }
    ...
    public ngOnInit(): void {
        if (!this.dtPicker) {
            throw Error(
                `DtDateTimePicker: the picker input doesn't have any associated date-time component`
            );
        }
    }
