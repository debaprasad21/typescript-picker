...
import { DATE_TIME_FORMATS } from './adapter/date-time-format.class';
...
describe('DateTimeComponent', () => {
    ...
    describe('with DTNativeDateTimeModule', () => {
        describe('standard DateTimePicker', () => {
            ...
            it('should open popup when pickerMode is "popup"', () => {
                expect(
                    document.querySelector('.cdk-overlay-pane.dt-popup')
                ).toBeNull();
            ...
            it('should open dateTimePicker if opened input is set to true', fakeAsync(() => {
                ...
                expect(
                    document.querySelector('.dt-container')
                ).not.toBeNull();
            ...
            it('should NOT open dateTimePicker if it is disabled', () => {
                ...
                expect(
                    document.querySelector('date-time-container')
                ).toBeNull();
            ...
            it('input should aria-owns date-time-container after opened in popup mode', fakeAsync(() => {
            ...
            it('input should aria-owns date-time-container after opened in dialog mode', fakeAsync(() => {
                testComponent.pickerMode = 'dialog';
                fixture.detectChanges();
            ...
            describe('set to multi-years', () => {
                ...
                it('should start at the multi-years view', () => {
                    ...
                    let multiYearTable = containerElement.querySelector(
                        'table.dt-calendar-multi-year-table'
                    );
        ...
        describe('DateTimePicker with dateTimeTrigger', () => {
            ...
            it('should not open the picker when trigger clicked if dateTimePicker is disabled', () => {
                ...
                expect(toggle.classList).toContain('dt-trigger-disabled');
                expect(
                    document.querySelector('date-time-container')
                ).toBeNull();
    ...
    describe('with missing DateTimeAdapter and DATE_TIME_FORMATS', () => {
        it('should throw when created', () => {
            expect(() => createComponent(StandardDateTimePicker)).toThrowError(
                /DtDateTimePicker: No provider found for .*/
            );
        });
    });
});
...
@Component({
    template: `
        <input [dtDateTime]="dt" [value]="date">
        <date-time [opened]="opened"
                       [disabled]="disabled"
                       [pickerType]="pickerType"
                       [pickerMode]="pickerMode" #dt></date-time>
    `
})
...
@Component({
    template: `
        <input [dtDateTime]="dt" [selectMode]="selectMode" [values]="dates">
        <date-time [startAt]="startAt"
                       [pickerType]="pickerType" #dt></date-time>
    `
})
...
@Component({
    template: `
        <input [dtDateTime]="dt" [value]="date">
        <date-time #dt [startAt]="startDate"></date-time>
    `
})
...
@Component({
    template: `
        <input [dtDateTime]="dt" [value]="date">
        <date-time #dt [startView]="startView"
                       (monthSelected)="onMonthSelection()"
                       (yearSelected)="onYearSelection()"></date-time>
    `
})
...
@Component({
    template: `
        <input [(ngModel)]="moment" [selectMode]="selectMode" [dtDateTime]="dt">
        <date-time #dt></date-time>
    `
})
...
@Component({
    template: `
        <input [dtDateTime]="dt">
        <button [dateTimeTrigger]="dt">Icon</button>
        <date-time #dt></date-time>
    `
})
...
@Component({
    template: `
        <input [(ngModel)]="date" [min]="min" [max]="max"
               [dtDateTime]="dt"
               [dateTimeTrigger]="dt">
        <date-time [showSecondsTimer]="true" #dt></date-time>
    `
})
...
@Component({
    template: `
    <input [dtDateTime]="dt" [(ngModel)]="value" [min]="min" [max]="max">
    <date-time #dt [startAt]="startAt"></date-time>
  `
})
...
@Component({
    template: `
    <input [(ngModel)]="selected" [dtDateTime]="dt">
    <date-time (afterPickerOpen)="openedSpy()" (afterPickerClosed)="closedSpy()" #dt></date-time>
  `
})
